  ...
  <div class="common-footer">
    <div class="common-tab-wrapper" v-for="(item, index) in tabs" :key="index" @click="onTabClick(item)">
      <div class="common-tab">
        <div class="tab-icon icon-discovery" :class="{'is-selected': isSelected(item.index)}" v-if="item.index === 1"></div>
        <div class="tab-icon icon-book3" :class="{'is-selected': isSelected(item.index)}" v-if="item.index === 2"></div>
        <div class="tab-icon icon-story" :class="{'is-selected': isSelected(item.index)}" v-if="item.index === 3"></div>
        <div class="tab-icon icon-profile" :class="{'is-selected': isSelected(item.index)}" v-if="item.index === 4"></div>
